...
exports.eejsBlock_editbarMenuLeft = (hook, args, cb) => {
  ...
  args.content += eejs.require('ep_ice_font_color/templates/editbarButtons.ejs');
  return cb();
};

exports.eejsBlock_dd_format = (hook, args, cb) => {
  args.content += eejs.require('ep_ice_font_color/templates/fileMenu.ejs');
  return cb();
};
...
exports.padInitToolbar = (hook, args, cb) => {
  ...
  fontColor.addOption('dummy', '颜色', {
    'data-l10n-id': 'ep_ice_font_color.color'
  });
  colors.forEach((color, value) => {
    fontColor.addOption(value, color, {
      'data-l10n-id': `ep_ice_font_color.${color}`
    });
  });
